...
const NumberInput = ({ label, value, onChange, onBlur, bgClass = "bg-white/40", textColor = "text-slate-800", disabled = false }) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <input 
        type="number" min="0" onFocus={handleFocus} onBlur={handleBlur} disabled={disabled} 
          ...
          <div className="min-w-0 flex-1">
            {isEditingName ? (
              <div className="flex items-center gap-2">
                <input autoFocus className="text-sm font-black uppercase bg-transparent border-b border-slate-500 text-slate-800 outline-none w-full" value={tempName} onChange={(e) => setTempName(e.target.value)} onBlur={handleSaveName} onKeyDown={(e) => { if (e.key === 'Enter') handleSaveName(); if (e.key === 'Escape') setIsEditingName(false); }} />
                <button onMouseDown={(e) => e.preventDefault()} onClick={handleSaveName} className="p-1 bg-white/40 rounded-full hover:bg-white/60 text-slate-800 shadow-md transition-all shrink-0"><Check size={14} /></button>
              </div>
            ) : (
...
const App = () => {
  ...
  const [searchDate, setSearchDate] = useState(''); // State untuk filter tanggal pencarian
  ...
  // 2. Auto Lock Session (5 Minutes Security)
  useEffect(() => {
    let timer;
      ...
      const decryptedData = snapshot.docs.map(doc => {
        ...
        if (isPatientLocked) {
          return {
            ...
            paymentStatus: realPayment, admissionDate: data.admissionDate || '',
  ...
  // --- C. MEMOIZED CALCULATIONS & FILTERING ---
  ...
  // --- E. SUB-RENDER FUNCTIONS (NAVIGATION & TABS) ---

  const navItems = [{ id: 'dashboard', label: 'Dashboard', icon: BarChart3, activeIconBg: 'bg-indigo-500' }];
  if (isStaffLoggedIn || isAdmin) { navItems.push({ id: 'input', label: 'Input Data', icon: LayoutDashboard, activeIconBg: 'bg-blue-500' }); }
  if ((!isStaffLoggedIn && !isAdmin)) { navItems.push({ id: 'search', label: 'Cari Pasien', icon: Search, activeIconBg: 'bg-pink-500' }); }
  if (isStaffLoggedIn || isAdmin) { navItems.push({ id: 'download', label: 'Laporan', icon: FileSpreadsheet, activeIconBg: 'bg-emerald-600' }); }
  if (isAdmin) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {!isStaffLoggedIn && (
            <div className="relative">
              <select className="w-full bg-white/50 p-4 rounded-2xl font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-100/50 transition-all appearance-none cursor-pointer text-xs uppercase border border-slate-100" value={searchRoomFilter} onChange={(e) => setSearchRoomFilter(e.target.value)}>
          ...
          <div className="relative">
            ...
            <input type="date" className="w-full bg-white/50 p-4 pl-12 rounded-2xl font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-100/50 transition-all text-xs border border-slate-100" value={searchDate} onChange={e => setSearchDate(e.target.value)} />
          </div>

          <div className={`relative ${!isStaffLoggedIn ? 'md:col-span-2' : 'md:col-span-3'}`}>
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
            <input type="text" placeholder={isStaffLoggedIn ? "Cari pasien di ruangan ini..." : "Ketik Nama Pasien..."} className="w-full bg-white/50 p-4 pl-12 rounded-2xl font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-100/50 transition-all text-sm border border-slate-100" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>
        ...
        <div className="mt-4">
          {searchResults.length > 0 ? (
            <div className="overflow-hidden border border-slate-200 rounded-2xl relative bg-white shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left border-collapse">
                  <thead className="bg-indigo-600 text-white font-bold uppercase tracking-wider">
                    <tr><th className="p-4">No</th><th className="p-4">Ruangan</th><th className="p-4">Nama</th><th className="p-4">JK</th><th className="p-4">Usia</th><th className="p-4">Alamat</th><th className="p-4">Tgl Masuk</th><th className="p-4">Tgl Keluar</th><th className="p-4">Status</th><th className="p-4">Keterangan</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {searchResults.map((p, i) => (
                      <tr key={p.id} className="hover:bg-indigo-50/50 transition-colors group">
                        ...
                        <td className="p-4 font-bold text-slate-800 border-r border-slate-100 whitespace-nowrap">{p.name}{p.jenisOperasi && <span className="block text-[9px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded w-fit mt-1">{p.jenisOperasi}</span>}{['IGD', 'IGD PONEK'].includes(p.room) && p.diagnosisPrimary && <span className="block text-[9px] text-slate-500 font-normal mt-1 max-w-[150px] truncate">{p.diagnosisPrimary}</span>}</td>
                        ...
                        <td className="p-4 border-r border-slate-100">
                          {p.room.toUpperCase().includes('POLI') ? (
                            ...
                            <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase whitespace-nowrap ${p.followUp === 'Dirawat' ? 'bg-emerald-100 text-emerald-700' : p.followUp === 'Meninggal di IGD' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-600'}`}>
                              {/* PERBAIKAN: Tampilkan ruangan jika Dirawat */}
                              {p.followUp === 'Dirawat' ? `Dirawat (${p.exitNote || '-'})` : p.followUp}
                            </span>
                          ) : (
                            <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase whitespace-nowrap ${p.status === 'Dirawat /Inap' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                              {/* PERBAIKAN: Tampilkan ruangan jika Pindah Ruangan */}
              ...
              <div className="bg-slate-50 p-3 border-t border-slate-200 text-right"><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total: {searchResults.length} Pasien Ditemukan</span></div>
            </div>
          ) : (
            <div className="text-center py-12 bg-white/30 rounded-[2rem] border border-dashed border-slate-300/50"><Users size={32} className="mx-auto text-slate-400/50 mb-3" /><p className="text-slate-400 text-xs font-bold uppercase tracking-wide">{(searchTerm || searchDate) ? "Tidak ada pasien yang sesuai kriteria" : "Belum ada data pasien"}</p></div>
          )}
        </div>
      </GlassContainer>
    </div>
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div className="bg-gradient-to-br from-blue-600 to-cyan-500 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-blue-500/20 relative overflow-hidden border border-white/10 group">
                ...
                <h2 className="text-xs font-black text-white/60 uppercase mb-4 tracking-widest flex justify-between items-center"><span>Pasien Rawat Inap</span><span className="text-white/80 bg-white/20 px-3 py-1 rounded-full">{formatDateID(report.date)}</span></h2>
                ...
                <div className="mt-8 bg-black/20 rounded-full h-3 w-full overflow-hidden backdrop-blur-sm"><div className="bg-white h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(totalBor, 100)}%` }}></div></div>
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-teal-400 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-emerald-500/20 relative overflow-hidden border border-white/10 group">
                ...
                <h2 className="text-xs font-black text-white/60 uppercase mb-4 tracking-widest flex justify-between items-center"><span>Kunjungan Poliklinik</span><span className="text-white/80 bg-white/20 px-3 py-1 rounded-full">{formatDateID(report.date)}</span></h2>
                ...
                <div className="mt-8 bg-white/10 rounded-xl p-3 backdrop-blur-sm border border-white/10"><div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-white/80"><span>Total Poli Buka</span><span>{poliRooms.length} Poli</span></div></div>
            </div>
        </div>
              ...
              <GlassContainer key={r.id} className="rounded-[2rem] p-5 flex flex-col justify-between hover:bg-white/80 transition-all cursor-default min-h-[140px]">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div><span className="text-[10px] font-black uppercase text-slate-700 tracking-wide block">{r.name}</span><span className={`text-[9px] font-black ${filled ? 'text-emerald-600' : 'text-slate-400'}`}>{filled ? (isPoli ? 'ADA KUNJUNGAN' : 'TERISI') : 'KOSONG'}</span></div>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 bg-slate-50 p-2 rounded-xl">
                    <div className="flex flex-col items-center"><span className="text-[8px] uppercase tracking-wider opacity-70">Total</span><span className="text-slate-800 text-sm font-black">{Number(finalReport[r.pasienKey] || 0)}</span></div>
                    <div className="w-px h-6 bg-slate-200"></div>
                    <div className="flex flex-col items-center"><span className="text-[8px] uppercase tracking-wider opacity-70">BPJS</span><span className="text-emerald-600 text-sm font-black">{Number(finalReport[r.bpjsKey] || 0)}</span></div>
                    <div className="w-px h-6 bg-slate-200"></div>
                    <div className="flex flex-col items-center"><span className="text-[8px] uppercase tracking-wider opacity-70">UMUM</span><span className="text-amber-600 text-sm font-black">{Number(finalReport[r.umumKey] || 0)}</span></div>
                  </div>
    ...
    <>
      ...
      <div className="flex h-screen w-full simas-animated-bg font-sans flex-col md:flex-row overflow-hidden text-slate-800 selection:bg-indigo-500 selection:text-white">
        ...
        {confirmModal.isOpen && (
          <div className="fixed inset-0 z-[6000] bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-6">
            <GlassContainer className="w-full max-w-sm rounded-[2.5rem] p-10 text-center !bg-white">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner ${confirmModal.type === 'save_patient' ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}><AlertTriangle size={40} /></div>
              ...
              <div className="flex gap-4">
                <button onClick={() => !isSaving && setConfirmModal({ isOpen: false })} disabled={isSaving} className="flex-1 p-4 bg-slate-50 rounded-2xl font-black uppercase text-[10px] text-slate-500 hover:bg-slate-100 transition-colors">Batal</button>
                <button onClick={handleConfirmAction} disabled={isSaving} className={`flex-1 p-4 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg transition-all ${confirmModal.type === 'save_patient' ? 'bg-emerald-500 shadow-emerald-500/30' : 'bg-rose-500 shadow-rose-500/30'}`}>{isSaving ? 'Memproses...' : 'Ya, Lanjut'}</button>
              </div>
            </GlassContainer>
          </div>
        )}
        ...
        {isUnlockModalOpen && (
          <div className="fixed inset-0 z-[6000] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-6">
            <GlassContainer className="w-full max-w-sm rounded-[2.5rem] p-10 text-center !bg-white">
              ...
              <input type="password" placeholder="Kunci Akses" className="w-full bg-slate-50 rounded-2xl py-3 px-4 font-bold text-center text-slate-700 outline-none border border-transparent focus:border-indigo-500 focus:bg-white transition-all mb-4" value={unlockKeyInput} onChange={e => setUnlockKeyInput(e.target.value)} autoFocus onKeyDown={(e) => { if (e.key === 'Enter') handleUnlockAkses(); }} />
              <div className="flex gap-4"><button onClick={() => setIsUnlockModalOpen(false)} className="flex-1 p-4 bg-slate-50 rounded-2xl font-black uppercase text-[10px] text-slate-500 hover:bg-slate-100 transition-colors">Batal</button><button onClick={handleUnlockAkses} className="flex-1 p-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 transition-all hover:scale-105">Buka</button></div>
            </GlassContainer>
          </div>
        )}
        ...
        {showLoginModal && (
          <div className="fixed inset-0 z-[6000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in-95 duration-300">
            <GlassContainer className="w-full max-w-sm rounded-[3rem] p-10 shadow-2xl relative text-center !bg-white">
              ...
              <div className="flex flex-col items-center mb-8"><div className="w-20 h-20 bg-indigo-100 rounded-3xl flex items-center justify-center text-indigo-600 mb-6 shadow-inner"><KeyRound size={40} /></div><h3 className="text-2xl font-black uppercase tracking-tighter text-slate-800">Login Petugas</h3><p className="text-xs font-bold text-slate-400 mt-2">Masukkan kredensial Anda</p></div>
        ...
        <aside className="hidden md:flex w-72 flex-col m-0 h-screen backdrop-blur-xl border-r border-white/30 shadow-2xl z-50 fixed left-0 top-0 bottom-0 glass-sidebar">
          <div className="p-8 pb-6 shrink-0"><div className="flex items-center gap-4 mb-3"><div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-600/20"><Activity size={28} /></div><div><h1 className="text-2xl font-black tracking-tighter uppercase leading-none">SIPAS</h1><span className="text-xs font-bold text-indigo-600 tracking-widest">RSUD LEBONG</span></div></div>{isStaffLoggedIn && loggedInRoomId && (<div className="mt-6 p-4 bg-indigo-50/60 rounded-2xl border border-indigo-100 flex flex-col gap-2 animate-in fade-in"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Login Sebagai</p><div className="flex justify-between items-center"><span className="text-sm font-black text-indigo-700">{rooms.find(r => r.id === loggedInRoomId)?.name}</span><button onClick={handleLogout} className="p-1.5 bg-white text-rose-500 rounded-lg shadow-sm hover:bg-rose-50 transition-colors"><LogOut size={14} /></button></div></div>)}{isAdmin && (<div className="mt-6 p-4 bg-indigo-700 rounded-2xl border border-indigo-600 flex flex-col gap-2 animate-in fade-in shadow-lg"><p className="text-[9px] font-black text-indigo-200 uppercase tracking-widest">Login Sebagai</p><div className="flex justify-between items-center"><span className="text-sm font-black text-white">SUPER ADMIN</span><button onClick={handleLogout} className="p-1.5 bg-indigo-600 text-white rounded-lg shadow-sm hover:bg-indigo-500 transition-colors"><LogOut size={14} /></button></div></div>)}{isInfoLoggedIn && (<div className="mt-6 p-4 bg-sky-50/60 rounded-2xl border border-sky-100 flex flex-col gap-2 animate-in fade-in"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Login Sebagai</p><div className="flex justify-between items-center"><span className="text-sm font-black text-sky-700">INFORMASI</span><button onClick={handleLogout} className="p-1.5 bg-white text-rose-500 rounded-lg shadow-sm hover:bg-rose-50 transition-colors"><LogOut size={14} /></button></div></div>)}</div>
          <nav className="flex-1 px-6 py-4 space-y-4 overflow-y-auto scrollbar-hide">{navItems.map((item) => (<button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center p-4 rounded-[1.5rem] transition-all duration-300 group ${activeTab === item.id ? 'bg-white shadow-xl shadow-indigo-100 text-indigo-600' : 'text-slate-700 hover:bg-white/50 hover:text-slate-900'}`}><div className={`w-10 h-10 rounded-xl flex items-center justify-center mr-4 transition-all ${activeTab === item.id ? `${item.activeIconBg} text-white shadow-lg` : 'bg-slate-100 text-slate-500 group-hover:bg-white'}`}><item.icon size={20} /></div><span className={`text-xs font-bold uppercase tracking-wide`}>{item.label}</span></button>)) }{!isStaffLoggedIn && !isAdmin && !isInfoLoggedIn && (<button onClick={() => setShowLoginModal(true)} className="w-full flex items-center p-4 rounded-[1.5rem] transition-all text-slate-700 hover:bg-white/80 hover:text-indigo-600 hover:shadow-lg"><div className="w-10 h-10 rounded-xl flex items-center justify-center mr-4 bg-slate-100 text-slate-500"><Lock size={20} /></div><span className="text-xs font-black uppercase tracking-widest">Login Petugas</span></button>)}</nav>
        </aside>

        <nav className="fixed bottom-6 left-6 right-6 h-20 bg-slate-900/90 backdrop-blur-2xl rounded-full shadow-2xl flex justify-around items-center border border-white/10 z-50 md:hidden px-2">{navItems.map((item) => (<button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center p-2 rounded-2xl transition-all duration-300 ${activeTab === item.id ? '-translate-y-6' : 'opacity-60'}`}><div className={`w-12 h-12 rounded-full flex items-center justify-center mb-1 transition-all shadow-lg ${activeTab === item.id ? `${item.activeIconBg} text-white ring-4 ring-slate-900` : 'bg-transparent text-slate-300 shadow-none'}`}><item.icon size={22} /></div>{activeTab === item.id && <span className="text-[9px] font-black uppercase tracking-tight text-slate-900 absolute -bottom-4 bg-white/90 backdrop-blur px-2 py-0.5 rounded-full shadow-sm">{item.label.split(' ')[0]}</span>}</button>))}{!isStaffLoggedIn && !isAdmin && !isInfoLoggedIn && (<button onClick={() => setShowLoginModal(true)} className={`flex flex-col items-center p-2 rounded-2xl transition-all duration-300 opacity-60 hover:opacity-100`}><div className="w-12 h-12 rounded-full flex items-center justify-center mb-1 transition-all shadow-none bg-transparent text-slate-300"><Lock size={22} /></div></button>)}</nav>
        ...
        <main className="flex-1 h-full overflow-hidden relative md:ml-72 flex flex-col">
          <header className="px-6 py-6 flex flex-col gap-4 bg-transparent shrink-0 z-40 md:hidden">
            <div className="flex justify-between items-center"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-white/50 backdrop-blur rounded-xl flex items-center justify-center shadow-lg text-indigo-700"><Activity size={24} /></div><div><h1 className="text-xl font-black tracking-tighter uppercase leading-none text-indigo-700">SIPAS</h1><span className="text-[10px] font-bold text-indigo-600 tracking-widest block">RSUD LEBONG</span></div></div><div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-wide flex items-center gap-2 backdrop-blur-md border border-white/30 shadow-lg ${dbStatus === 'online' ? 'bg-emerald-100/80 text-emerald-700' : 'bg-red-100/80 text-red-700'}`}><div className={`w-2 h-2 rounded-full ${dbStatus === 'online' ? 'bg-emerald-500' : 'bg-red-500'}`}></div>{dbStatus}</div></div>
            {(isStaffLoggedIn || isAdmin || isInfoLoggedIn) && (<div className="flex justify-between items-center bg-white/40 backdrop-blur-md p-3 rounded-2xl border border-white/40 shadow-sm animate-in slide-in-from-top-2"><div className="flex items-center gap-2"><div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600"><User size={16} /></div><div><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Login</p><h2 className="text-xs font-black text-indigo-700 uppercase">{isAdmin ? 'SUPER ADMIN' : isInfoLoggedIn ? 'INFORMASI' : rooms.find(r => r.id === loggedInRoomId)?.name}</h2></div></div><button onClick={handleLogout} className="p-2 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm"><LogOut size={16} /></button></div>)}
          </header>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-32 md:pb-10 scrollbar-hide">
            <div className="max-w-7xl mx-auto w-full space-y-6">
                          ...
                          <div className="flex items-center gap-4 w-full md:w-auto">
                            {isAdmin && (
                              <div className="relative w-full md:w-48">
                                <select className="w-full bg-slate-50 pl-4 pr-10 py-3 rounded-2xl text-xs font-black border-2 border-transparent focus:border-indigo-100 outline-none text-slate-700 shadow-inner appearance-none uppercase" value={adminSelectedRoomId} onChange={(e) => setAdminSelectedRoomId(e.target.value)}><option value="">-- PILIH RUANGAN --</option>{rooms.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select>
                                <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 rotate-90 pointer-events-none" size={16} />
                              </div>
                            )}
                            <div className="relative w-full md:w-auto flex-1"><CalendarDays className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-500" size={18} /><input type="date" value={report.date} onChange={e => setReport({ ...report, date: e.target.value })} className="w-full bg-slate-50 pl-12 pr-6 py-3 rounded-2xl text-base font-black border-2 border-transparent focus:border-indigo-100 outline-none text-slate-700 shadow-inner" /></div>
                          </div>
                        </div>
                      </GlassContainer>
                      <div className={`grid grid-cols-1 ${isAdmin && !adminSelectedRoomId ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-6`}>{visibleRooms.map(room => (<RoomCard key={room.id} room={room} report={finalReport} onChange={(k, v) => setReport(p => ({ ...p, [k]: v }))} onSave={() => handleSaveReport()} isAdmin={isAdmin} onDeleteRoom={(id) => setConfirmModal({ isOpen: true, type: 'room', id, message: 'Hapus ruangan ini?' })} onUpdateName={handleUpdateRoomName} />))}</div>
                      <div className="mt-12 border-t border-white/30 pt-8">
                        <div className="flex flex-col gap-4 mb-8">
                          <div className="flex justify-between items-center"><h3 className="text-xl font-black text-slate-700 uppercase tracking-tighter flex items-center gap-3"><div className="bg-indigo-100 p-2 rounded-xl text-indigo-600"><Users size={24} /></div>Daftar Pasien</h3><div className="flex items-center gap-2"><button onClick={() => setIsLocalSearchVisible(!isLocalSearchVisible)} className={`p-3 rounded-full transition-all ${isLocalSearchVisible ? 'bg-indigo-100 text-indigo-600' : 'bg-white text-slate-400 hover:text-indigo-500 shadow-sm'}`}><Search size={20} /></button><button type="button" onClick={handleOpenAddModal} className="bg-blue-600 text-white px-6 py-3 rounded-full font-bold text-xs uppercase tracking-wide shadow-xl hover:scale-105 transition-transform flex items-center gap-3"><UserPlus size={18} /> Tambah</button></div></div>
                          {isLocalSearchVisible && (<div className="animate-in slide-in-from-top-2 fade-in"><input type="text" placeholder="Cari nama pasien..." className="w-full p-4 rounded-2xl border border-slate-200 focus:border-indigo-500 outline-none font-bold text-slate-700" value={localSearchTerm} onChange={(e) => setLocalSearchTerm(e.target.value)} autoFocus /></div>)}
                        </div>
                        <div className="grid gap-4">
                          {roomPatients.length === 0 ? (<div className="text-center py-16 bg-white/40 rounded-[2.5rem] border-2 border-dashed border-slate-200/60"><Users size={48} className="mx-auto text-slate-300 mb-4" /><p className="text-slate-400 font-bold text-sm uppercase tracking-wide">{localSearchTerm ? `Tidak ada pasien dengan nama "${localSearchTerm}"` : (isAdmin && !adminSelectedRoomId ? "Pilih ruangan untuk melihat/input pasien" : "Belum ada data pasien di ruangan ini")}</p></div>) : (roomPatients.map(p => (<GlassContainer key={p.id} className="rounded-3xl p-5 flex justify-between items-center group hover:border-indigo-200 transition-colors"><div className="flex items-center gap-5"><div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-lg ${p.gender === 'Laki-Laki' ? 'bg-blue-500 text-white' : 'bg-rose-500 text-white'}`}>{p.name.charAt(0)}</div><div><p className="font-black text-slate-800 text-lg">{p.name}</p><p className="text-xs text-slate-500 mt-1">{p.address || 'Alamat tidak diisi'}</p><div className="flex gap-2 mt-2 items-center flex-wrap"><span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-lg font-bold text-slate-500 uppercase">{p.gender}</span><span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-lg font-bold text-slate-500 uppercase">{p.age} Thn</span>{['OK (BEDAH SENTRAL)', 'KAMAR OPERASI'].some(r => p.room.toUpperCase().includes(r)) ? (<><span className="text-[10px] bg-purple-100 text-purple-600 px-2 py-0.5 rounded-lg font-bold uppercase">{p.specialization}</span><span className="text-[10px] bg-pink-100 text-pink-600 px-2 py-0.5 rounded-lg font-bold uppercase">{p.operationCategory}</span></>) : p.room.toUpperCase().includes('POLI') ? (<span className="text-[10px] bg-orange-100 text-orange-600 px-2 py-0.5 rounded-lg font-bold uppercase">{p.visitType}</span>) : (<span className={`text-[10px] px-2 py-0.5 rounded-lg font-bold uppercase ${p.status === 'Dirawat /Inap' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-500'}`}>{p.status}</span>)}<span className={`text-[10px] px-2 py-0.5 rounded-lg font-bold uppercase ${p.paymentStatus === 'BPJS' ? 'bg-sky-100 text-sky-600' : p.paymentStatus === 'Keringanan (Cost Sharing)' ? 'bg-purple-100 text-purple-600' : 'bg-amber-100 text-amber-600'}`}>{p.paymentStatus}</span></div></div></div><div className="flex flex-col items-end gap-2"><div className="flex gap-2"><button onClick={() => { setEditingPatient(p); setPatientFormData(p); setIsPatientModalOpen(true); }} className="p-3 bg-indigo-50 rounded-xl text-indigo-600 hover:bg-indigo-600 hover:text-white transition-colors shadow-sm"><Edit3 size={18} /></button><button onClick={() => setConfirmModal({ isOpen: true, type: 'patient', id: p.id, message: `Hapus pasien ${p.name}?` })} className="p-3 bg-rose-50 rounded-xl text-rose-600 hover:bg-rose-600 hover:text-white transition-colors shadow-sm"><Trash2 size={18} /></button></div><span className="text-[9px] font-black text-slate-400 bg-slate-50 px-3 py-1 rounded-full">{['OK (BEDAH SENTRAL)', 'KAMAR OPERASI'].some(r => p.room.toUpperCase().includes(r)) ? 'Operasi' : 'Masuk'}: {formatDateID(p.admissionDate)}</span></div></GlassContainer>)))}
                        </div>
                      </div>
                    </>
                  ) : (
                    <GlassContainer className="rounded-[2.5rem] p-8 text-center mb-8 border-dashed border-2 border-indigo-200 bg-indigo-50/30"><h2 className="text-2xl font-black text-slate-400 uppercase tracking-widest mb-2">Area Terbatas</h2><p className="text-slate-400 font-bold mb-6 text-sm">Silakan login untuk mengakses input data ruangan</p></GlassContainer>
                  )}
                </div>
              )}
              ...
              {activeTab === 'download' && (
                <div className="h-full flex flex-col gap-6 pb-20">
                  <GlassContainer className="p-8 rounded-[2.5rem] flex flex-col gap-6 h-full overflow-hidden">
                    <div className="flex flex-col gap-6 border-b border-slate-200 pb-6"><div className="flex flex-col md:flex-row justify-between items-end gap-4"><div><h2 className="text-2xl font-black text-slate-800 flex items-center gap-3"><FileSpreadsheet className="text-blue-600" />Export Data</h2><p className="text-slate-500 text-sm mt-1">Pratinjau dan unduh data laporan pasien.</p></div><div className="flex items-center gap-4">{isAdmin && (<div className="relative"><select className="appearance-none bg-slate-100 border border-slate-200 text-slate-700 py-3 pl-4 pr-10 rounded-xl font-bold text-xs focus:ring-2 focus:ring-blue-500 outline-none uppercase" value={downloadRoomFilter} onChange={(e) => setDownloadRoomFilter(e.target.value)}><option value="">-- SEMUA RUANGAN --</option>{rooms.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select><ChevronRight className="absolute right-3 top-1/2 -translate-y-1/2 rotate-90 text-slate-400 pointer-events-none" size={14} /></div>)}<button onClick={handleDownloadExcel} className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold text-xs shadow-lg shadow-emerald-500/30 hover:bg-emerald-700 transition-all flex items-center gap-2"><Download size={16} /> DOWNLOAD EXCEL</button></div></div><div className="bg-slate-50 p-4 rounded-2xl flex flex-col md:flex-row gap-4 items-center border border-slate-200"><div className="flex items-center gap-2 w-full md:w-auto"><div className="bg-white p-2 rounded-xl border border-slate-200 text-slate-400"><CalendarRange size={18} /></div><div className="flex flex-col"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Bulan Laporan</span><input type="month" className="bg-transparent font-bold text-slate-700 text-sm outline-none w-full" value={downloadSettings.month} onChange={(e) => setDownloadSettings({...downloadSettings, month: e.target.value})}/></div></div><div className="h-8 w-px bg-slate-200 hidden md:block"></div><div className="flex flex-col md:flex-row gap-2 w-full"><label className={`flex-1 p-3 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-3 ${downloadSettings.mode === 'mtd' ? 'border-emerald-500 bg-emerald-50' : 'border-transparent bg-white hover:bg-slate-100'}`}><input type="radio" name="downloadMode" className="hidden" checked={downloadSettings.mode === 'mtd'} onChange={() => setDownloadSettings({...downloadSettings, mode: 'mtd'})} /><div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${downloadSettings.mode === 'mtd' ? 'border-emerald-500' : 'border-slate-300'}`}>{downloadSettings.mode === 'mtd' && <div className="w-2 h-2 bg-emerald-500 rounded-full" />}</div><div className="flex flex-col"><span className={`text-xs font-black uppercase ${downloadSettings.mode === 'mtd' ? 'text-emerald-700' : 'text-slate-600'}`}>Rekapan Awal Bulan s/d Hari Ini</span><span className="text-[9px] font-bold text-slate-400">Data Real Time (s/d {new Date().getDate()} {new Date().toLocaleString('id-ID', { month: 'short' })})</span></div></label><label className={`flex-1 p-3 rounded-xl border-2 cursor-pointer transition-all flex items-center gap-3 ${downloadSettings.mode === 'full' ? 'border-indigo-500 bg-indigo-50' : 'border-transparent bg-white hover:bg-slate-100'}`}><input type="radio" name="downloadMode" className="hidden" checked={downloadSettings.mode === 'full'} onChange={() => setDownloadSettings({...downloadSettings, mode: 'full'})} /><div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${downloadSettings.mode === 'full' ? 'border-indigo-500' : 'border-slate-300'}`}>{downloadSettings.mode === 'full' && <div className="w-2 h-2 bg-indigo-500 rounded-full" />}</div><div className="flex flex-col"><span className={`text-xs font-black uppercase ${downloadSettings.mode === 'full' ? 'text-indigo-700' : 'text-slate-600'}`}>Rekapan Satu Bulan Penuh</span><span className="text-[9px] font-bold text-slate-400">Semua Data di Bulan Terpilih</span></div></label></div></div></div>
                    <div className="flex-1 overflow-auto border border-slate-200 rounded-2xl relative bg-white shadow-sm">
                      <table className="w-full text-xs text-left border-collapse"><thead className="bg-indigo-600 text-white font-bold uppercase tracking-wider"><tr><th className="p-4">No</th><th className="p-4">Ruangan</th><th className="p-4">Nama</th><th className="p-4">JK</th><th className="p-4">Usia</th><th className="p-4">Alamat</th><th className="p-4">Tgl Masuk</th><th className="p-4">Tgl Keluar</th><th className="p-4">Status</th><th className="p-4">Keterangan</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredDownloadData.length > 0 ? (filteredDownloadData.map((p, i) => (<tr key={p.id} className="hover:bg-indigo-50/50 transition-colors group"><td className="p-4 font-bold text-slate-500 text-center border-r border-slate-100">{i + 1}</td><td className="p-4 border-r border-slate-100 whitespace-nowrap">{p.room}</td><td className="p-4 font-bold text-slate-800 border-r border-slate-100 whitespace-nowrap">{p.name}</td><td className="p-4 border-r border-slate-100 text-center">{p.gender === 'Laki-Laki' ? 'L' : 'P'}</td><td className="p-4 text-center border-r border-slate-100">{p.age} Th</td><td className="p-4 border-r border-slate-100 max-w-[150px] truncate" title={p.address}>{p.address || '-'}</td><td className="p-4 border-r border-slate-100 whitespace-nowrap">{formatDateID(p.admissionDate)}</td><td className="p-4 border-r border-slate-100 whitespace-nowrap">{p.outcomeDate ? formatDateID(p.outcomeDate) : '-'}</td><td className="p-4 border-r border-slate-100 text-center"><span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase whitespace-nowrap ${p.paymentStatus === 'BPJS' ? 'bg-sky-100 text-sky-700' : p.paymentStatus === 'Keringanan (Cost Sharing)' ? 'bg-purple-100 text-purple-700' : 'bg-amber-100 text-amber-700'}`}>{p.paymentStatus}</span></td><td className="p-4 border-r border-slate-100">{p.room.includes('POLI') ? (p.followUpService || p.diagnosisPrimary) : (p.status || p.followUp)}</td></tr>))) : (<tr><td colSpan="10" className="p-10 text-center text-slate-400 font-bold">Tidak ada data untuk filter bulan/jenis rekapan ini.</td></tr>)}</tbody></table>
                    </div>
              ...
              {activeTab === 'history' && (
                <div className="space-y-6 animate-in slide-in-from-right duration-500 pb-32"><GlassContainer className="p-8 rounded-[2.5rem] flex justify-between items-center"><h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Arsip Laporan</h2><div className="w-14 h-14 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-500"><ClipboardList size={28} /></div></GlassContainer><div className="grid gap-4">{savedReports.map(item => (<GlassContainer key={item.id} className="p-6 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-6 hover:border-indigo-300 transition-colors group"><div className="flex items-center gap-6"><div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-lg shadow-indigo-500/30">{new Date(item.date).getDate()}</div><div><h4 className="font-black text-slate-800 uppercase text-lg tracking-tight">{formatDateID(item.date)}</h4><p className="text-xs text-slate-400 font-bold uppercase tracking-widest opacity-80 mt-1">Diinput: {new Date(item.createdAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</p></div></div><div className="flex items-center gap-3"><button onClick={() => { setEditFormData(item); setEditingId(item.id); }} className="p-3 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm"><Edit3 size={20} /></button><button onClick={() => setConfirmModal({ isOpen: true, type: 'report', id: item.id, message: 'Hapus data laporan ini secara permanen?' })} className="p-3 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm"><Trash2 size={20} /></button></div></GlassContainer>))}</div></div>
              )}

              {activeTab === 'settings' && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500 pb-32">
                  <GlassContainer className="p-10 rounded-[3rem] shadow-2xl relative overflow-hidden"><div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter mb-8 flex items-center gap-4 relative z-10"><div className="bg-indigo-100 p-3 rounded-2xl text-indigo-600"><ShieldCheck size={28} /></div> Keamanan Admin</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10"><div className="space-y-4"><h5 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Identitas Admin</h5><div className="grid grid-cols-2 gap-4"><input type="text" className="w-full bg-white/50 rounded-2xl p-5 font-bold text-sm outline-none border border-slate-200 focus:border-indigo-500 text-slate-700 transition-colors" value={authSettings.adminUsername} onChange={e => setAuthSettings({ ...authSettings, adminUsername: e.target.value })} /><input type="password" className="w-full bg-white/50 rounded-2xl p-5 font-bold outline-none border border-slate-200 focus:border-indigo-500 text-slate-700 transition-colors" value={authSettings.adminPassword} onChange={e => setAuthSettings({ ...authSettings, adminPassword: e.target.value })} /></div><button onClick={() => updateCredentials(authSettings)} className="w-full bg-blue-600 text-white p-5 rounded-2xl font-black uppercase text-xs shadow-xl active:scale-95 hover:bg-black transition-all">Update Admin</button></div></div>
                    <div className="mt-12 pt-10 border-t border-slate-200 relative z-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end"><div className="space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">ID Ruangan (Tanpa Spasi)</span><input type="text" className="w-full bg-white/50 rounded-2xl p-4 font-bold text-xs outline-none border border-slate-200 focus:border-indigo-500 text-slate-700" placeholder="CONTOH: MELATI" value={newRoomData.id} onChange={e => setNewRoomData({ ...newRoomData, id: e.target.value.toUpperCase().replace(/\s/g, '') })} /></div><div className="space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">Nama Ruangan</span><input type="text" className="w-full bg-white/50 rounded-2xl p-4 font-bold text-xs outline-none border border-slate-200 focus:border-indigo-500 text-slate-700" placeholder="Ruang Melati" value={newRoomData.name} onChange={e => setNewRoomData({ ...newRoomData, name: e.target.value })} /></div><button onClick={handleAddRoom} className="bg-indigo-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-xl active:scale-95 hover:bg-indigo-700 transition-all flex justify-center items-center gap-2"><Plus size={18} /> Tambah</button></div>
                    </div>
                    <div className="mt-12 pt-10 border-t border-slate-200 relative z-10"><h5 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 ml-1">Manajemen Akun Petugas</h5>
                      <div className="p-6 bg-sky-50/50 rounded-[2rem] flex flex-col md:flex-row gap-6 items-center border border-sky-100 shadow-sm mb-6"><div className="w-full md:w-1/4"><label className="text-xs font-black uppercase text-sky-700 block mb-1">Petugas Informasi</label><span className="text-[10px] bg-sky-100 text-sky-500 font-bold px-2 py-1 rounded-lg">ID: informasi</span></div><div className="flex gap-4 w-full md:w-3/4 items-end"><div className="flex-1 space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">Password Akses</span><input type="text" className="w-full bg-white rounded-xl p-3 text-xs font-bold border border-slate-100 outline-none focus:ring-2 focus:ring-sky-100" value={authSettings.infoPassword || '123'} onChange={e => setAuthSettings({ ...authSettings, infoPassword: e.target.value })} /></div><button onClick={() => updateCredentials(authSettings)} className="bg-sky-500 text-white p-3 rounded-xl shadow-lg shadow-sky-500/30 hover:bg-sky-600 transition-colors"><Save size={18} /></button></div></div>
                      <div className="p-6 bg-emerald-50/50 rounded-[2rem] flex flex-col md:flex-row gap-6 items-center border border-emerald-100 shadow-sm mb-6"><div className="w-full md:w-1/4"><label className="text-xs font-black uppercase text-emerald-700 block mb-1">Kode Akses Buka Nama</label><span className="text-[10px] bg-emerald-100 text-emerald-500 font-bold px-2 py-1 rounded-lg">Utk: Tombol Gembok</span></div><div className="flex gap-4 w-full md:w-3/4 items-end"><div className="flex-1 space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">Kode Rahasia</span><input type="text" className="w-full bg-white rounded-xl p-3 text-xs font-bold border border-slate-100 outline-none focus:ring-2 focus:ring-emerald-100" value={authSettings.publicAccessCode || '123'} onChange={e => setAuthSettings({ ...authSettings, publicAccessCode: e.target.value })} /></div><button onClick={() => updateCredentials(authSettings)} className="bg-emerald-500 text-white p-3 rounded-xl shadow-lg shadow-emerald-500/30 hover:bg-emerald-600 transition-colors"><Save size={18} /></button></div></div>
                      <div className="grid grid-cols-1 gap-4">{rooms.map(room => { const creds = authSettings.roomAccess?.[room.id] || { username: room.id.toLowerCase(), password: '123' }; return (<div key={room.id} className="p-6 bg-white/40 rounded-[2rem] flex flex-col md:flex-row gap-6 items-center border border-white/60 shadow-sm hover:shadow-md transition-all"><div className="w-full md:w-1/4"><label className="text-xs font-black uppercase text-indigo-700 block mb-1">{room.name}</label><span className="text-[10px] bg-indigo-50 text-indigo-400 font-bold px-2 py-1 rounded-lg">ID: {room.id}</span></div><div className="flex gap-4 w-full md:w-3/4 items-end"><div className="flex-1 space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">Nama Ruangan</span><input type="text" className="w-full bg-white rounded-xl p-3 text-xs font-bold border border-slate-100 outline-none focus:ring-2 focus:ring-indigo-100" value={roomNameEdits[room.id] || room.name} onChange={e => setRoomNameEdits({ ...roomNameEdits, [room.id]: e.target.value })} /></div><div className="flex-1 space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">Username</span><input type="text" className="w-full bg-white rounded-xl p-3 text-xs font-bold border border-slate-100 outline-none focus:ring-2 focus:ring-indigo-100" value={creds.username} onChange={e => { const newUsername = e.target.value; setAuthSettings(prev => ({ ...prev, roomAccess: { ...prev.roomAccess, [room.id]: { ...creds, username: newUsername } } })) }} /></div><div className="flex-1 space-y-2"><span className="text-[9px] font-black text-slate-400 uppercase ml-1">Password</span><input type="text" className="w-full bg-white rounded-xl p-3 text-xs font-bold border border-slate-100 outline-none focus:ring-2 focus:ring-indigo-100" value={creds.password} onChange={e => { const newPass = e.target.value; setAuthSettings(prev => ({ ...prev, roomAccess: { ...prev.roomAccess, [room.id]: { ...creds, password: newPass } } })) }} /></div><button onClick={() => saveRoomConfiguration(room.id, roomNameEdits[room.id] || room.name, creds.username, creds.password)} className="bg-emerald-500 text-white p-3 rounded-xl shadow-lg shadow-emerald-500/30 hover:bg-emerald-600 transition-colors"><Save size={18} /></button></div></div>)})}</div>
                    </div>
                  </GlassContainer>
                </div>
              )}
        ...
        {isPatientModalOpen && (
          <div className="fixed inset-0 z-[5000] bg-white flex flex-col animate-in slide-in-from-bottom-4 duration-300">
            <div className="shrink-0 px-6 md:px-10 py-5 border-b border-slate-200 flex justify-between items-center bg-white shadow-sm z-50"><div><h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-slate-800 flex items-center gap-3"><div className="bg-indigo-600 text-white p-2 rounded-lg"><UserPlus size={24} /></div>Data Pasien</h2><p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1 ml-1">{patientFormData.room ? `Input Data Ruangan: ${patientFormData.room}` : 'Formulir Pasien Baru'}</p></div><div className="flex items-center gap-4"><button onClick={() => setIsPatientModalOpen(false)} className="p-3 bg-slate-100 hover:bg-rose-100 text-slate-600 hover:text-rose-600 rounded-full transition-all group"><X size={24} className="group-hover:rotate-90 transition-transform" /></button></div></div>
                      ...
                      {patientFormData.room.toUpperCase().includes('POLI') ? (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                          <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-6"><div className="space-y-2"><label className="text-[11px] font-bold text-slate-500 uppercase ml-1">Tanggal Kunjungan</label><input required type="date" className="w-full bg-slate-50 focus:bg-white rounded-2xl py-4 px-5 text-sm font-bold border border-slate-200 outline-none focus:border-emerald-500 transition-all" value={patientFormData.admissionDate} onChange={e => setPatientFormData({ ...patientFormData, admissionDate: e.target.value })} /></div><div className="space-y-2"><label className="text-[11px] font-bold text-slate-500 uppercase ml-1">No. RM</label><input required className="w-full bg-slate-50 focus:bg-white rounded-2xl py-4 px-5 text-sm font-bold border border-slate-200 outline-none focus:border-emerald-500 transition-all" value={patientFormData.mrn} onChange={e => setPatientFormData({ ...patientFormData, mrn: e.target.value })} placeholder="00-00-00" /></div></div>
                          ...
                          {['IGD', 'IGD PONEK'].includes(patientFormData.room) ? (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                              ...
                              <div className="space-y-6">
                                ...
                                <div className="space-y-2"><label className="text-[11px] font-bold text-slate-500 uppercase ml-1">Diagnosa Pra/Pasca Bedah</label><textarea rows="2" className="w-full bg-slate-50 focus:bg-white rounded-2xl py-4 px-5 text-sm font-bold border border-slate-200 outline-none focus:border-purple-500 resize-none" value={patientFormData.diagnosisPrimary} onChange={e => setPatientFormData({ ...patientFormData, diagnosisPrimary: e.target.value })} placeholder="Diagnosa Utama"></textarea></div>
                                <div className="space-y-2"><label className="text-[11px] font-bold text-slate-500 uppercase ml-1">Tindak Operasi</label><textarea rows="2" className="w-full bg-slate-50 focus:bg-white rounded-2xl py-4 px-5 text-sm font-bold border border-slate-200 outline-none focus:border-purple-500 resize-none" value={patientFormData.action} onChange={e => setPatientFormData({ ...patientFormData, action: e.target.value })} placeholder="Jenis Tindakan yang dilakukan"></textarea></div>
                              </div>
                            </div>
                          ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                              ...
                              <div className="space-y-6">
                                ...
                                <div className="space-y-2"><label className="text-[11px] font-bold text-slate-500 uppercase ml-1">Tindak Lanjut</label><select className="w-full bg-slate-50 focus:bg-white rounded-2xl py-4 px-5 text-sm font-bold border border-slate-200 outline-none focus:border-indigo-500" value={patientFormData.followUp} onChange={e => setPatientFormData({ ...patientFormData, followUp: e.target.value })}><option value="Dirawat">Dirawat</option><option value="Dirujuk">Dirujuk</option><option value="Pulang">Pulang</option></select></div>
